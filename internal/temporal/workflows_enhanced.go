...
func EnhancedTCRWorkflow(ctx workflow.Context, input EnhancedTCRInput) (*EnhancedTCRResult, error) {
	...
	result.WorkflowState = StateBootstrap
	...
	if err != nil {
		...
		result.WorkflowState = StateFailed
		return result, nil
	}
	...
	result.WorkflowState = StateGenTest
	...
	if err != nil || !genTestResult.Passed {
		result.Gates["gen_test"] = *genTestResult
	...
	result.WorkflowState = StateLintTest
	gateStart = workflow.Now(ctx)
	...
	if err != nil || !lintTestResult.Passed {
		result.Gates["lint_test"] = *lintTestResult
	...
	result.WorkflowState = StateVerifyRED
	gateStart = workflow.Now(ctx)
	...
	if err != nil || !verifyRedResult.Passed {
		result.Gates["verify_red"] = *verifyRedResult
	...
	result.WorkflowState = StateGenImpl
	gateStart = workflow.Now(ctx)
	...
	if err != nil || !genImplResult.Passed {
		result.Gates["gen_impl"] = *genImplResult
	...
	result.WorkflowState = StateVerifyGREEN
	gateStart = workflow.Now(ctx)
	...
	if err != nil || !verifyGreenResult.Passed {
		result.Gates["verify_green"] = *verifyGreenResult
	...
	result.WorkflowState = StateMultiReview
	gateStart = workflow.Now(ctx)
	...
	if err != nil || !multiReviewResult.Passed {
		result.Gates["multi_review"] = *multiReviewResult
		result.ReviewVotes = multiReviewResult.ReviewVotes
	...
	result.WorkflowState = StateCommit
	...
	err = workflow.ExecuteActivity(ctx, enhancedActivities.GetCommitSHA, bootstrap).Get(ctx, &commitSHA)
	if err == nil {
