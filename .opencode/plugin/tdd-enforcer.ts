  ...
  return {
    ...
    event: async ({ event }) => {
      ...
      if (event.type === "session.created") {
        ...
        await client.tui.showToast({
          body: {
            message: "🧪 TDD Enforcer: Write tests BEFORE implementation",
            variant: "info"
          }
        })
      }
      ...
      if (event.type === "session.idle") {
        if (sessionStats.testsWritten > 0 || sessionStats.implsWritten > 0) {
          ...
          const status = sessionStats.violations === 0 ? "✅ No violations" : `${sessionStats.violations} violations`
      ...
      if (event.type === "file.edited") {
        ...
        if (isTestFile(filePath)) {
          ...
          if (existsSync(fullImplPath)) {
            ...
            await showSuccess(`Test written first - following TDD! ✨`)
          }
        }
          ...
          if (!existsSync(fullTestPath)) {
            ...
            if (state) {
              if (!state.usesTestify) {
                ...
                console.log(`[TDD Enforcer] ✅ Test-first development - good job!`)
              }
            } else {
